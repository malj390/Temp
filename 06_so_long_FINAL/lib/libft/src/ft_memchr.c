...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/28 08:56:36 by mlermo-j          #+#    #+#             */
/*   Updated: 2025/04/28 08:56:38 by mlermo-j         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// #include <stdio.h>
// // 	if (!s) is like if (s == NULL)
// int	main(void)
// {
// 	char s[] = "search the point. And everything that follows";
// 	char 	*r;

// 	r = ft_memchr((void *)s, '.', 20);
// 	printf("%s", r);
// 	return (0);
// }

/*
MEMCHR(3)			      Linux Programmer's Manual
MEMCHR(3)
...
SYNOPSIS
       ...
       void _memchr(const void _s, int c, size_t n);

       void _memrchr(const void _s, int c, size_t n);

       void _rawmemchr(const void _s, int c);
...
DESCRIPTION
       ...
       except that it searches backward from the end of the n bytes
       pointed to by s instead of forward from the beginning.

       The rawmemchr() function is similar to memchr(): it assumes
       (i.e., the  programmer  knows  for certain)  that  an
       instance  of  c lies somewhere in the memory area starting at
       the location pointed to by s, and so performs an optimized
       search for c (i.e., no use of a	count  argument to  limit
       the range of the search).  If an instance of c is not found,
       the results are unpre‐ dictable.  The following call is
       a fast means of locating a string's terminating null byte:

	   char _p = rawmemchr(s, '\0');
...
RETURN VALUE
       ...
       The  rawmemchr()  function  returns  a  pointer	to the
       matching byte, if one is found.	If no matching byte is found,
